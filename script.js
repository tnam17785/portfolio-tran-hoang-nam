(() => {
 ...
 if (typeof matrixCanvas !== 'undefined' && matrixCanvas) { matrixCanvas.width = width; matrixCanvas.height = height; }
 }
 class Particle {
 ...
 particles = [];
 const count = Math.min(CONFIG.particleCount, Math.floor((width * height) / 10000) || 40);
 ...
 initParticles(); addTerminalLine('overdrive terminated'); updateHUD();
 }
 ...
 { id: 'home', label: 'Go Home', desc: 'Top', run: () => document.getElementById('home') && document.getElementById('home').scrollIntoView({ behavior: 'smooth' }) },
 { id: 'about', label: 'Go About', desc: '01', run: () => document.getElementById('about') && document.getElementById('about').scrollIntoView({ behavior: 'smooth' }) },
 ...
 playBeep(880, 0.08);
 if (secretsFound >= SECRET_TOTAL) {
 document.body.classList.add('all-secrets');
 showToast('ALL SECRETS', '7/7 unlocked');
 for (let i = 0; i < 25; i++) setTimeout(() => { for (let j = 0; j < 4; j++) bursts.push(new BurstParticle(Math.random() * width, Math.random() * height * 0.5)); }, i * 50);
 }
 renderAch();
 }
 updateSecretsUI();
 ...
 graffitiMarks.push({ x: x, y: y, r: 8 + Math.random() * 20, color: colors[Math.floor(Math.random() * colors.length)], life: 1, rot: Math.random() * Math.PI });
 }
 ...
 let perfMode = false, totalClicks = 0, lastActivity = Date.now(), idleNotified = false;
 const btnPerf = document.getElementById('btn-perf');
 function setPerf(on) {
 perfMode = on;
 if (btnPerf) { btnPerf.classList.toggle('active', on); btnPerf.textContent = on ? 'PERF ON' : 'PERF'; }
 const hp = document.getElementById('hud-perf'); if (hp) hp.textContent = on ? 'ON' : 'OFF';
 if (on) {
 CONFIG.particleCount = 45; CONFIG.maxDist = 90;
 trailDots.forEach(d => { d.el.style.display = 'none'; });
 registerSecret('perf', 'OPTIMIZED', 'Perf');
 showToast('PERF', 'Fewer particles');
 } else {
 CONFIG.particleCount = overdrive ? 160 : 110; CONFIG.maxDist = overdrive ? 160 : 130;
 trailDots.forEach(d => { d.el.style.display = ''; });
 showToast('PERF', 'Full quality');
 }
 initParticles(); updateHUD();
 }
 if (btnPerf) btnPerf.addEventListener('click', () => setPerf(!perfMode));
 window.addEventListener('mousedown', () => {
 totalClicks++; lastActivity = Date.now(); idleNotified = false;
 const e = document.getElementById('hud-clicks'); if (e) e.textContent = totalClicks;
 });
 document.addEventListener('mousemove', () => { lastActivity = Date.now(); idleNotified = false; });
 document.addEventListener('keydown', e => {
 lastActivity = Date.now(); idleNotified = false;
 if (e.code === 'Space' && document.activeElement.tagName !== 'INPUT') {
 e.preventDefault();
 const x = mouse.x != null ? mouse.x : width / 2, y = mouse.y != null ? mouse.y : height / 2;
 ...
 e.preventDefault();
 const o = document.getElementById('ach-overlay');
 if (o) { if (o.classList.contains('open')) o.classList.remove('open'); else openAch(); }
 }
 });
 setInterval(() => {
 ...
 if (achClose) achClose.addEventListener('click', () => { const o = document.getElementById('ach-overlay'); if (o) o.classList.remove('open'); });
 const achO = document.getElementById('ach-overlay');
 if (achO) achO.addEventListener('click', e => { if (e.target === achO) achO.classList.remove('open'); });
