/* THN Portfolio v5 — see local for comments */
(() => {
  'use strict';
  let overdrive = false;
  ...
  document.addEventListener('mousemove', e => {
    ...
    if (dt > 0 && dt < 50) {
      ...
      if (dist / dt > 4.5) {
        ...
        triggerPageGlitch();
        clearTimeout(window._stormT);
        window._stormT = setTimeout(() => document.body.classList.remove('glitch-storm'), 200);
        registerSecret('storm', 'GLITCH STORM', 'Mouse velocity');
      }
    }
